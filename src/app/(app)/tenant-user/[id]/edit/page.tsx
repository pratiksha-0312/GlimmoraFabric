...
export default function TenantUserEditPage() {
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      const { usersApi } = await import("@/lib/api");
      // Backend admin-update accepts {full_name, role, is_active,
      // email_verified}. Email + password updates use the dedicated
      // self-service flows; mark them TODO here.
      void username;
      void mfa;
      void password;
      ...
      /* swallow — list refresh handles state */
    }
    setSaving(false);
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <button onClick={() => router.push("/tenant-user")} className="mt-4 flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium text-white" style={{ backgroundColor: "var(--gf-accent)" }}>
  ...
  return (
    <AuthGuard allowedRoles={["tenant_admin"] as UserRole[]}>
      <div className="space-y-6 max-w-2xl">
        <button onClick={() => router.push("/tenant-user")} className="flex items-center gap-2 text-sm font-medium transition-colors hover:opacity-70" style={{ color: "var(--gf-text-secondary)" }}>
          <ArrowLeft className="h-4 w-4" />Back to Users
        </button>
        ...
        <div className="rounded-2xl border shadow-2xl" style={{ backgroundColor: "var(--gf-bg-surface)", borderColor: "var(--gf-border)" }}>
          ...
          <div className="flex items-center justify-between border-b px-6 py-4" style={{ borderColor: "var(--gf-border)" }}>
            ...
            <button onClick={() => router.push("/tenant-user")} className="rounded-lg p-1 transition-colors hover:opacity-70" style={{ color: "var(--gf-text-secondary)" }}>
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button type="button" onClick={() => router.push("/tenant-user")}
