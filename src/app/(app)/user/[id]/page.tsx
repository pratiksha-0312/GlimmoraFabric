  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Map FastAPI User -> local PlatformUser. Password is never exposed
        // by the backend; surface an empty string so the input renders the
        // bullet placeholder.
        setUser({
          id: u.id,
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <button onClick={() => router.push("/user")} className="mt-4 flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium text-white" style={{ backgroundColor: "var(--gf-accent)" }}>
  ...
  return (
    <AuthGuard allowedRoles={["super_admin"] as UserRole[]}>
      <div className="space-y-6 max-w-2xl">
        <button onClick={() => router.push("/user")} className="flex items-center gap-2 text-sm font-medium transition-colors hover:opacity-70" style={{ color: "var(--gf-text-secondary)" }}>
          <ArrowLeft className="h-4 w-4" />Back to Users
        </button>
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" value="Asia/Kolkata" readOnly className="w-full rounded-lg border px-3 py-2 text-sm outline-none opacity-70 cursor-not-allowed" style={fieldStyle} />
              </div>
            </div>
