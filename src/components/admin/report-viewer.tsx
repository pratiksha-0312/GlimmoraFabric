...
export function ReportViewer({ reportId }: { reportId: string }) {
  const report = REPORTS[reportId] ?? { ...DEFAULT_REPORT, id: reportId, name: `Report ${reportId}` };
  ...
  const handleDownload = () => {
    const lines = [
      ...
      "DETAILED FINDINGS",
      "-".repeat(40),
      ...report.sections.map((s) => `\n[${s.status.toUpperCase()}] ${s.name} — ${s.score}%\n${s.details}`),
    ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Link href="/admin/compliance/reports" className="flex items-center justify-center rounded-lg border h-9 w-9 hover:opacity-70" style={{ borderColor: "var(--gf-border)", color: "var(--gf-text-secondary)" }}>
