...
// ---------------------------------------------------------------------------
// Edit Tenant Wizard (modal popup) — mirrors Create wizard, prefilled, title only differs
// ---------------------------------------------------------------------------

function EditTenantWizard({ tenantId, onClose }: { tenantId: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const cfgPlan = cfg.plan as Tenant["plan"] | undefined;
  ...
  const readOnlyStyle = { backgroundColor: "var(--gf-bg-elevated)", borderColor: "var(--gf-border)", color: "var(--gf-text-muted)" };
  const selectedPlanData = PLAN_DATA.find((p) => p.name === plan)!;
  ...
  const toggleFeature = (f: string) => {
    setFeatures((prev) => { const n = new Set(prev); n.has(f) ? n.delete(f) : n.add(f); return n; });
  };
  ...
  return (
    <ModalOverlay onClose={onClose} wide>
      ...
      <div className="px-6 py-4 border-b" style={{ borderColor: "var(--gf-border)" }}>
        <div className="flex items-center justify-between">
          {WIZARD_STEPS.map((s, i) => (
            <div key={s.num} className="flex items-center gap-2 flex-1">
              <div className="flex items-center gap-2">
                <div className="flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold transition-colors" style={{ backgroundColor: step >= s.num ? "var(--gf-accent)" : "var(--gf-bg-elevated)", color: step >= s.num ? "#fff" : "var(--gf-text-muted)" }}>
                  {step > s.num ? <Check className="h-4 w-4" /> : s.num}
                </div>
                <span className="text-xs font-medium" style={{ color: step >= s.num ? "var(--gf-text-primary)" : "var(--gf-text-muted)" }}>{s.label}</span>
              </div>
              {i < WIZARD_STEPS.length - 1 && <div className="flex-1 h-px mx-2" style={{ backgroundColor: step > s.num ? "var(--gf-accent)" : "var(--gf-border)" }} />}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-6 py-5">
        {step === 1 && (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--gf-text-secondary)" }}>Tenant Code</label>
              <input type="text" value={tenantCode} readOnly className="w-full rounded-lg border px-3 py-2.5 text-sm outline-none cursor-not-allowed" style={readOnlyStyle} />
              <p className="text-[10px] mt-1" style={{ color: "var(--gf-text-muted)" }}>Auto-generated, cannot be changed</p>
            </div>
            <div>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--gf-text-secondary)" }}>Tenant Full Name *</label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Acme Corporation" className="w-full rounded-lg border px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[var(--gf-accent)]/40" style={{ ...fieldStyle, ...(errors.name ? { borderColor: "#ef4444" } : {}) }} />
              {errors.name && <p className="text-xs text-red-500 mt-1">{errors.name}</p>}
            </div>
            <div>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--gf-text-secondary)" }}>Contact Email *</label>
              <input type="email" value={username} onChange={(e) => setUsername(e.target.value.trim().toLowerCase())} placeholder="e.g. admin@acme.com" className="w-full rounded-lg border px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[var(--gf-accent)]/40" style={{ ...fieldStyle, ...(errors.username ? { borderColor: "#ef4444" } : {}) }} />
              {errors.username && <p className="text-xs text-red-500 mt-1">{errors.username}</p>}
            </div>
            <div>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--gf-text-secondary)" }}>Role</label>
              <input type="text" value="Tenant Admin" readOnly className="w-full rounded-lg border px-3 py-2.5 text-sm outline-none cursor-not-allowed" style={readOnlyStyle} />
            </div>
            <div>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--gf-text-secondary)" }}>Password</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Leave blank to keep current password" className="w-full rounded-lg border px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[var(--gf-accent)]/40" style={fieldStyle} />
            </div>
            <div>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--gf-text-secondary)" }}>Confirm Password</label>
              <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Confirm password" className="w-full rounded-lg border px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[var(--gf-accent)]/40" style={{ ...fieldStyle, ...(errors.confirmPassword ? { borderColor: "#ef4444" } : {}) }} />
              {errors.confirmPassword && <p className="text-xs text-red-500 mt-1">{errors.confirmPassword}</p>}
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-5">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                {PLAN_DATA.map((p) => (
                  <button key={p.name} type="button" onClick={() => handlePlanSelect(p.name)} className="relative rounded-xl border-2 p-4 text-left transition-all hover:opacity-90" style={{ borderColor: plan === p.name ? p.color : "var(--gf-border)", backgroundColor: plan === p.name ? `${p.color}10` : "var(--gf-bg-base)" }}>
                    {plan === p.name && <div className="absolute top-2 right-2 flex h-5 w-5 items-center justify-center rounded-full" style={{ backgroundColor: p.color }}><Check className="h-3 w-3 text-white" /></div>}
                    <p className="text-xs font-medium mb-1" style={{ color: p.color }}>{p.name}</p>
                    <p className="text-lg font-bold" style={{ color: "var(--gf-text-primary)" }}>{p.price}<span className="text-xs font-normal" style={{ color: "var(--gf-text-muted)" }}>{p.period}</span></p>
                    <p className="text-[10px] mt-1" style={{ color: "var(--gf-text-muted)" }}>{p.description}</p>
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex rounded-lg border overflow-hidden" style={{ borderColor: "var(--gf-border)" }}>
                {(["Monthly", "Annual"] as const).map((c) => (
                  <button key={c} type="button" onClick={() => setBillingCycle(c)} className="flex-1 px-4 py-2.5 text-sm font-medium transition-colors" style={{ backgroundColor: billingCycle === c ? "var(--gf-accent)" : "var(--gf-bg-base)", color: billingCycle === c ? "#fff" : "var(--gf-text-secondary)" }}>
                    {c}{c === "Annual" && <span className="ml-1 text-[10px] opacity-80">(Save 20%)</span>}
                  </button>
                ))}
              </div>
            </div>
            <div>
              ...
              <p className="text-[10px] mt-1" style={{ color: "var(--gf-text-muted)" }}>Default for {plan}: {selectedPlanData.maxUsers} users</p>
              {errors.maxUsers && <p className="text-xs text-red-500 mt-1">{errors.maxUsers}</p>}
            </div>
            <div>
              <label className="block text-xs font-medium mb-2" style={{ color: "var(--gf-text-secondary)" }}>Features</label>
              <div className="grid grid-cols-2 gap-2">
                {FEATURES.map((f) => (
                  <label key={f} className="flex items-center gap-2.5 rounded-lg border px-3 py-2.5 cursor-pointer transition-colors hover:opacity-90" style={{ borderColor: features.has(f) ? "var(--gf-accent)" : "var(--gf-border)", backgroundColor: features.has(f) ? "rgba(var(--gf-accent-rgb, 249, 115, 22), 0.08)" : "var(--gf-bg-base)" }}>
                    <input type="checkbox" checked={features.has(f)} onChange={() => toggleFeature(f)} className="accent-[var(--gf-accent)] h-3.5 w-3.5" />
                    <span className="text-sm" style={{ color: "var(--gf-text-primary)" }}>{f}</span>
                  </label>
                ))}
        ...
        {step === 3 && (
          <div className="space-y-5">
            ...
            <div className="rounded-xl border p-4 space-y-3" style={{ borderColor: "var(--gf-border)", backgroundColor: "var(--gf-bg-base)" }}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><p className="text-[10px] uppercase tracking-wide" style={{ color: "var(--gf-text-muted)" }}>Tenant Code</p><p className="text-sm font-medium font-mono" style={{ color: "var(--gf-text-primary)" }}>{tenantCode}</p></div>
                <div><p className="text-[10px] uppercase tracking-wide" style={{ color: "var(--gf-text-muted)" }}>Full Name</p><p className="text-sm font-medium" style={{ color: "var(--gf-text-primary)" }}>{name}</p></div>
                <div><p className="text-[10px] uppercase tracking-wide" style={{ color: "var(--gf-text-muted)" }}>Contact Email</p><p className="text-sm font-medium" style={{ color: "var(--gf-text-primary)" }}>{username}</p></div>
                <div><p className="text-[10px] uppercase tracking-wide" style={{ color: "var(--gf-text-muted)" }}>Role</p><p className="text-sm font-medium" style={{ color: "var(--gf-text-primary)" }}>Tenant Admin</p></div>
              </div>
            </div>
            <div className="rounded-xl border p-4 space-y-3" style={{ borderColor: "var(--gf-border)", backgroundColor: "var(--gf-bg-base)" }}>
              <div className="flex items-center justify-between">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><p className="text-[10px] uppercase tracking-wide" style={{ color: "var(--gf-text-muted)" }}>Plan</p><span className="inline-block px-2.5 py-0.5 rounded-full text-xs font-medium mt-0.5" style={{ backgroundColor: PLAN_BADGE_STYLES[plan].bg, color: PLAN_BADGE_STYLES[plan].color }}>{plan} — {billingCycle}</span></div>
                <div><p className="text-[10px] uppercase tracking-wide" style={{ color: "var(--gf-text-muted)" }}>Max Users</p><p className="text-sm font-medium" style={{ color: "var(--gf-text-primary)" }}>{maxUsers}</p></div>
                <div className="col-span-2">
                  <p className="text-[10px] uppercase tracking-wide" style={{ color: "var(--gf-text-muted)" }}>Features</p>
                  <div className="flex flex-wrap gap-1 mt-0.5">
                    {Array.from(features).map((f) => (<span key={f} className="inline-block px-2 py-0.5 rounded-full text-[10px] font-medium" style={{ backgroundColor: "var(--gf-bg-elevated)", color: "var(--gf-text-secondary)" }}>{f}</span>))}
                    {features.size === 0 && <span className="text-sm" style={{ color: "var(--gf-text-muted)" }}>None</span>}
                  </div>
                </div>
              </div>
      ...
      <div className="flex items-center justify-between border-t px-6 py-4" style={{ borderColor: "var(--gf-border)" }}>
        <button type="button" onClick={step === 1 ? onClose : handleBack} className="rounded-lg px-4 py-2 text-sm font-medium border transition-colors hover:opacity-80" style={{ borderColor: "var(--gf-border)", color: "var(--gf-text-primary)" }}>
          {step === 1 ? "Cancel" : "Back"}
        </button>
        {step < 3 ? (
          <button type="button" onClick={handleNext} className="rounded-lg px-6 py-2 text-sm font-semibold text-white transition-colors hover:opacity-90" style={{ backgroundColor: "var(--gf-accent)" }}>Next</button>
        ) : (
          <button type="button" onClick={handleSave} className="rounded-lg px-6 py-2 text-sm font-semibold text-white transition-colors hover:opacity-90" style={{ backgroundColor: "var(--gf-accent)" }}>Save Changes</button>
        )}
      </div>
    </ModalOverlay>
  );
...
// ---------------------------------------------------------------------------
// View Tenant Wizard (modal popup) — readonly
// ---------------------------------------------------------------------------

function ViewTenantWizard({ tenantId, onClose }: { tenantId: string; onClose: () => void }) {
  const [tenantCode, setTenantCode] = useState("");
  const [step, setStep] = useState(1);
  const [name, setName] = useState("");
  const [username, setUsername] = useState("");
    ...
    <ModalOverlay onClose={onClose} wide>
      ...
      <div className="px-6 py-5">
        {step === 1 && (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--gf-text-secondary)" }}>Tenant Full Name *</label>
              <input type="text" value={name} readOnly className="w-full rounded-lg border px-3 py-2.5 text-sm outline-none cursor-not-allowed" style={readOnlyStyle} />
            </div>
            <div>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--gf-text-secondary)" }}>Contact Email *</label>
              <input type="email" value={username} readOnly className="w-full rounded-lg border px-3 py-2.5 text-sm outline-none cursor-not-allowed" style={readOnlyStyle} />
            </div>
            <div>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--gf-text-secondary)" }}>Role</label>
              <input type="text" value="Tenant Admin" readOnly className="w-full rounded-lg border px-3 py-2.5 text-sm outline-none cursor-not-allowed" style={readOnlyStyle} />
            ...
            <div>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--gf-text-secondary)" }}>Password *</label>
              <input type="password" value="••••••••" readOnly className="w-full rounded-lg border px-3 py-2.5 text-sm outline-none cursor-not-allowed" style={readOnlyStyle} />
            </div>
            <div>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--gf-text-secondary)" }}>Confirm Password *</label>
              <input type="password" value="••••••••" readOnly className="w-full rounded-lg border px-3 py-2.5 text-sm outline-none cursor-not-allowed" style={readOnlyStyle} />
        ...
        {step === 2 && (
          <div className="space-y-5">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                {PLAN_DATA.map((p) => (
                  <button key={p.name} type="button" disabled className="relative rounded-xl border-2 p-4 text-left cursor-not-allowed" style={{ borderColor: plan === p.name ? p.color : "var(--gf-border)", backgroundColor: plan === p.name ? `${p.color}10` : "var(--gf-bg-base)" }}>
                    {plan === p.name && <div className="absolute top-2 right-2 flex h-5 w-5 items-center justify-center rounded-full" style={{ backgroundColor: p.color }}><Check className="h-3 w-3 text-white" /></div>}
                    <p className="text-xs font-medium mb-1" style={{ color: p.color }}>{p.name}</p>
                    <p className="text-lg font-bold" style={{ color: "var(--gf-text-primary)" }}>{p.price}<span className="text-xs font-normal" style={{ color: "var(--gf-text-muted)" }}>{p.period}</span></p>
                    <p className="text-[10px] mt-1" style={{ color: "var(--gf-text-muted)" }}>{p.description}</p>
            ...
            <div>
              ...
              <div className="flex rounded-lg border overflow-hidden" style={{ borderColor: "var(--gf-border)" }}>
                {(["Monthly", "Annual"] as const).map((c) => (
                  <button key={c} type="button" disabled className="flex-1 px-4 py-2.5 text-sm font-medium cursor-not-allowed" style={{ backgroundColor: billingCycle === c ? "var(--gf-accent)" : "var(--gf-bg-base)", color: billingCycle === c ? "#fff" : "var(--gf-text-secondary)" }}>
                    {c}{c === "Annual" && <span className="ml-1 text-[10px] opacity-80">(Save 20%)</span>}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--gf-text-secondary)" }}>Max Users</label>
              <input type="number" min={1} value={maxUsers} readOnly className="w-full rounded-lg border px-3 py-2.5 text-sm outline-none cursor-not-allowed" style={readOnlyStyle} />
              <p className="text-[10px] mt-1" style={{ color: "var(--gf-text-muted)" }}>Default for {plan}: {selectedPlanData.maxUsers} users</p>
            </div>
            <div>
              <label className="block text-xs font-medium mb-2" style={{ color: "var(--gf-text-secondary)" }}>Features</label>
              <div className="grid grid-cols-2 gap-2">
                {FEATURES.map((f) => (
                  <label key={f} className="flex items-center gap-2.5 rounded-lg border px-3 py-2.5 cursor-not-allowed" style={{ borderColor: features.has(f) ? "var(--gf-accent)" : "var(--gf-border)", backgroundColor: features.has(f) ? "rgba(var(--gf-accent-rgb, 249, 115, 22), 0.08)" : "var(--gf-bg-base)" }}>
      ...
      <div className="flex items-center justify-between border-t px-6 py-4" style={{ borderColor: "var(--gf-border)" }}>
        ...
        {step < 3 ? (
          ...
          <button type="button" onClick={onClose} className="rounded-lg px-6 py-2 text-sm font-semibold text-white transition-colors hover:opacity-90" style={{ backgroundColor: "var(--gf-accent)" }}>Close</button>
        )}
      </div>
    </ModalOverlay>
  );
      ...
      <div className="px-6 py-5">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: "Contact Email", value: tenant.username },
        ...
        {tab === "settings" && (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--gf-text-secondary)" }}>Contact Email</label>
              <input type="email" value={sUsername} onChange={(e) => setSUsername(e.target.value)} className="w-full rounded-lg border px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[var(--gf-accent)]/40" style={fieldStyle} />
            </div>
            <div>
...
export function TenantsContent() {
  ...
  return (
    <div className="space-y-6">
                    ...
                    <tr key={tenant.id} className="border-b last:border-0 hover:bg-black/5 dark:hover:bg-white/5 transition-colors" style={{ borderColor: "var(--gf-border)" }}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => setViewTenantId(tenant.id)} className="rounded-lg p-1.5 transition-colors hover:bg-blue-500/10" style={{ color: "#3b82f6" }} title="View Details">
      ...
      {editTenantId && (
        <EditTenantWizard tenantId={editTenantId} onClose={() => setEditTenantId(null)} />
      )}

      {viewTenantId && (
        <ViewTenantWizard tenantId={viewTenantId} onClose={() => setViewTenantId(null)} />
      )}
    </div>
  );
}
